...
import { ChevronDown, LayoutDashboard, ShoppingBag, Store, Package, FileSpreadsheet, LogOut, Wrench, Moon, Sun } from 'lucide-react';
...
const SidebarLink = ({ to, icon: Icon, children }: { to: string, icon: React.ElementType, children: React.ReactNode }) => {
  ...
  return (
    <NavLink
      to={to}
      className={cn(
        "flex items-center gap-3 px-3 py-2 transition-all duration-200 text-sm font-bold uppercase tracking-widest",
        isActive 
...
const SidebarSubLink = ({ to, children }: { to: string, children: React.ReactNode }) => {
  ...
  return (
    <NavLink
      to={to}
      className={cn(
        "ml-8 flex items-center px-3 py-2 text-[11px] font-bold uppercase tracking-widest transition-all duration-200",
        isActive
...
export const Layout = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex font-sans text-gray-900 dark:bg-[#0F1115] dark:text-[#E0E2E6]">
      {/* Sidebar */}
      <aside className="w-64 bg-stone-50 border-r border-gray-200 flex-shrink-0 fixed h-full z-10 flex flex-col dark:bg-[#090A0C] dark:border-[#2A2D35]">
        <div className="h-20 flex items-center px-6 border-b border-gray-200 dark:border-[#2A2D35]">
          <div className="flex items-center gap-2 text-[#F27D26] font-black text-xl italic tracking-tighter uppercase p-2">
            <Wrench size={24} className="text-[#F27D26]" />
            <span>HOKY TEKNIK</span>
          </div>
        </div>
        
        <div className="flex-1 overflow-y-auto p-4 py-8 flex flex-col gap-1">
          ...
          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 mt-6 px-3 dark:text-[#8E9299]">Penjualan & Operasional</div>
          ...
          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 mt-6 px-3 dark:text-[#8E9299]">Analitik</div>
          <div className="px-3 py-2">
            <div className="flex items-center justify-between text-sm font-bold uppercase tracking-widest text-gray-600 dark:text-[#E0E2E6]">
              <span className="flex items-center gap-3">
        ...
        <div className="p-4 border-t border-gray-200 space-y-3 dark:border-[#2A2D35]">
          <button
            type="button"
      ...
      <main className="flex-1 ml-64 flex flex-col min-h-screen overflow-x-hidden">
        <header className="p-8 flex justify-between items-end border-b border-gray-200 z-0 sticky top-0 bg-stone-50 dark:border-[#2A2D35] dark:bg-[#090A0C]">
          <div>
